...
class Zoo_visitor_member_auth extends Zoo_visitor_lib {
	...
	public function member_login()
	{
		...
		*/
		...
		{
			return array('login' => lang('mbr_form_empty'));
		}

		// This should go in the auth lib.
		if ( ! $this->EE->auth->check_require_ip())
		{
			return array('login' => lang('unauthorized_request'));
		}
	 ...
	 */
	function _check_min_unpwd($member_obj, $username, $password)
	{
	 ...
	 */
	function _do_auth($username, $password)
	{
		...
		if ( ! $sess)
		{
			...
			{
				return array('login' => lang('invalid_existing_un_pw'));
			}
		}
	...
	function _build_multi_success_message($sites)
	{
		// Figure out return
	 ...
	 * Build Success Message
	 */
	function _build_success_message($sites)
	{
		// Build success message
	 ...
	 */
	function _update_online_user_stats()
	{
