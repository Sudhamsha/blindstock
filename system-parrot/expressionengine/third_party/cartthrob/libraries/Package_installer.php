...
class Package_installer
{
	...
	private $subtypes = array(
		'channel' => array('field_group', 'categories', 'channel_data'),
		'template_group' => array('template'),
		'field_group' => array('field'),
		'categories' => array('category'),
		'channel_data'	=> array('channel_data')
 	);
	 ...
	 * @subpackage CT Template Installer
	 */
	private function create_template_group($template_group, $params = array() )
	{
	 ...
	 * @param boolean $add_data If TRUE this will add channel data to be installed
	 * @return void
	 * @since 1.0.0
	 * @author Rob Sanchez | Chris Newton
	 * @subpackage CT Template Installer
	 */
	private function create_channel($channel, $params = array())
	{
	...
	private function create_channel_entry($channel)
	{
		...
		$this->EE->load->model("packages_entries_model");
		$this->EE->load->model('channel_model'); 
		
		foreach ($channel->channel_entry as $entry_xml)
		{
			$entry= array(); 
			$query = $this->EE->channel_model->get_channels(NULL, array(), array(array('channel_name' => array($channel_data['channel_name']))));
			...
			foreach ($entry_xml->field as $field)
			{
 			...
 			$this->EE->packages_entries_model->create_entry($entry); 
		}
	}
	// END
	...
	public function install($params = array() )
	{
		foreach ($this->packages as $package)
		{
			...
			{
				call_user_func(array($this, 'create_'.$package->getName()), $package, $params);
			}
		}
	}
	
	public function install_templates($params = array() )
	{
		foreach ($this->packages as $package)
		{
	...
	public function install_channels($params = array() )
	{
 		foreach ($this->packages as $package)
		{
