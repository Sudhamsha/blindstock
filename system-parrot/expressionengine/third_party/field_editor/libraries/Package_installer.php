...
class Package_installer
{
	...
	private function create_field($field)
	{
		...
		if ( ! in_array($field_data['field_type'], $installed_fieldtypes))
		{
			return $this->log_error('fieldtype_not_installed', $field_data['field_type'], $original_name);
		}
		...
		if (method_exists($this, 'create_field_'.$field_data['field_type']) && isset($field_data[$field_data['field_type']]) && $field_type_data = @unserialize(base64_decode($field_data[$field_data['field_type']])))
		{
	...
	private function create_field_grid($field_id, $field_type_data, $field_data)
	{
		...
		{
                        if(isset($row['col_id'])){unset($row['col_id']);} //avoid duplicate key
	...
	private function create_field_matrix($field_id, $field_type_data, &$field_data)
	{
		...
		foreach ($field_type_data as $row)
		{
			...
			$this->EE->dbforge->add_column('matrix_data', array('col_id_'.$col_id => array('type' => 'text', 'null' => FALSE)));
		}
