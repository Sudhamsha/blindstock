...
class Snippet_sync_ext {
    ...
    public function Snippet_sync_ext($settings = '')
    {
        ...
        // Shouldn't be necessary, but appears some add-ons don't terminate their
        // AR query properly. http://boldminded.com/support/ticket/382
        ...
        $this->path = reduce_double_slashes($this->path);
    ...
    public function settings()
    {
        ...
        $settings['suffix_as_separator'] = array('s', array('y' => 'Yes', 'n' => 'No'));
    ...
    public function update_extension($current = '')
    {
        ...
        // Change priority to something prior to User module, b/c it's screwing with the sessions end $session var
        if ($current < '1.3.2') 
        { 
            $this->EE->db->where('class', __CLASS__);
            $this->EE->db->update('extensions', array('priority' => 2));
        }
    ...
    private function _do_sync()
    {
        ...
        if($dir)
        {
            foreach($dir as $k => $v)
            {
                ...
                { 
                    if ($this->_sync_snippet($k, $this->path, $v))
                    {
                        $updated = TRUE;
                    }
                }
                else // it's a site-directory
                {
                    ...
                    foreach($v as $dk => $dv)
                    {
                        ...
                        {
                            if ($this->_sync_snippet($dk, $this->path . $k .'/', $dv))
                            {
                                $updated = TRUE;
                            }
                        }
                        else // it's a sub-directory
                        {
                            ...
                            {
                                if ($this->_sync_snippet($dk, $this->path . $k.'/'.$dk .'/', $sub_dv, $sub_prefix.$dk))
                                {
                                    $updated = TRUE;
                                }
                            }
        ...
        if($updated AND $this->EE->extensions->active_hook('snippet_sync_updated'))
        {
    ...
    private function _sync_snippet($key, $path, $file, $sub_folder = false)
    {
        ...
        $check_hash = md5_file($path . $file);
        $snippet_name = $this->_generate_prefix($sub_folder) . basename($file, '.html');

        // If a suffix is defined, use it as the separator for all special characters.
        if(
            (isset($this->settings['subfolder_suffix']) && $this->settings['subfolder_suffix'] != '') &&
            (isset($this->settings['suffix_as_separator']) && $this->settings['suffix_as_separator'] == 'y')
        ){
            $snippet_name = preg_replace('/[^a-zA-Z0-9:]/', $this->settings['subfolder_suffix'], $snippet_name);
        }

        // var_dump($snippet_name);
        // var_dump(md5_file($path . $file));
        // var_dump($this->settings['hashes'][$this->site_id][$snippet_name][0]);
        ...
        if(
            ...
            || ( 
                $check_hash != $this->settings['hashes'][$this->site_id][$snippet_name][0]
                && ($this->settings['hashes'][$this->site_id][$snippet_name][1] < $filemtime) 
            )
        ) 
        {
            $result = TRUE;
