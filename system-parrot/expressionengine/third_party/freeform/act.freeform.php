...
class Freeform_actions extends Addon_builder_freeform
{
	...
	{
		$parser_shim = 'Addon_builder_parser_' . $this->lower_name;
		$template_set = (isset(ee()->TMPL) && is_object(ee()->TMPL));
		...
		if ($capture_old OR ! $template_set)
		{
			if ($capture_old)
			{
				$this->old_tmpl = ee()->TMPL;
			}

			if ( ! class_exists($parser_shim))
			{
				require_once $this->addon_path . 'addon_builder/parser.addon_builder.php';
			}

			ee()->TMPL = new $parser_shim();
		}
