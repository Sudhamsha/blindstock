...
class Field_editor_mcp
{
	...
	public function edit($errors = FALSE, $existing_fields = FALSE)
	{
		...
		//since Text inputs are always the default, we can and should apply default settings
//		$blank_vars['field_type_settings'] = $this->EE->javascript->generate_json(array(
//			array(
//				 'name' => 'field_maxl',
//				 'value' => '128',
//			),
...
//		), TRUE);
		...
		$this->EE->cp->add_to_head('<script type="text/javascript" src="'.URL_THIRD_THEMES.'field_editor/js/jquery.settingsTable.js"></script>');
		
		$this->EE->cp->add_to_head('<script type="text/javascript" src="'.URL_THIRD_THEMES.'field_editor/js/fieldEditor.js"></script>');
		...
		$this->EE->javascript->output('
			...
			), JSON_FORCE_OBJECT).', {"rowCallbacks":'.$callbacks.'}));
                        console.log("field_editor",fieldEditor);    
			fieldEditor.table.settingsTable({
				...
				deleteConfirm: '.json_encode(lang('field_editor_delete_field_confirm'), JSON_FORCE_OBJECT).',
	...
	private function update_field($field_data, $keep_existing_settings = FALSE)
	{
		...
		{
                        //http://expressionengine.stackexchange.com/questions/21254/field-editor-unable-to-load-the-requested-class-services-json
